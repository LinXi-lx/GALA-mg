...
{ // 1 warp per vertex
    ...
    while (warp_id < vertex_num)
    {
        ...
        for (int i = warp_size / 2; i >= 1; i = i / 2)
        {
            ...
            // degree += __shfl_xor(degree, i, WARP_SIZE);
        }
        if (lane_id == 0)
        {
...
{ // insert into hashmap
    ...
    { // global+share
        ...
        {                                                // this position is vacant
            ...
            cur_weight = prev_weight + neighbor_weight;
            // printf("%d:%d\n",neighbor_com,cur_weight);
            double dq = 0;
            if (neighbor_com != cur_com)
            {
                dq = (double)(2.0 * (double)cur_weight - 2.0 * (double)Ki * ((double)tot_neighbor_com - (double)tot_cur_com + (double)Ki) * constant);
            }
            //  if ((dq > *maxdq) || ((dq == *maxdq) && (dq != 0) && Tot[neighbor_com]> Tot[*best_com]))
            if ((dq > *maxdq) || ((dq == *maxdq) && (dq != 0) && (neighbor_com < *best_com)))
            { // dq can equal 0
                *maxdq = dq;
                *best_com = neighbor_com;
...
{ // find in hashmap
    ...
    { // no global
        ...
        do
        {
            ...
            // }while(i<table_size&&neighbor_com_ids[pos]!=0);
        } while (i < shared_size);
    }
    else
    { // global+share
...
{ // for each neighbor,compute gain
    ...
    for (int i = warp_size / 2; i >= 1; i = i / 2)
    {
        ...
        // if ((recv_dq > maxdq) || (recv_dq == maxdq && Tot[recv_com]> Tot[best_com]))//!!!!!!!!!!!!!!!!!
    ...
    {
        // if (com_size[best_com] == 1 && com_size[cur_com] == 1 && Tot[best_com] <= Tot[cur_com])
...
{ // for each neighbor,compute gain
    ...
    // delete vertex u from its community

    // cur_community[vertex]=-1;//!!!!!!!!!!!
    ...
    {
        // if(cur_com_weight<0) printf("%d %d\n",warp_size,cur_com_weight);
        In[vertex] = cur_com_weight; // atomic??????
    }
    ...
    { //???
        ...
        __shared__ double shared_to_compare[64]; // may exceed share mem limit??????? warp_numPerBlk=4
...
{ // warp_size=threadAlloc
    ...
    for (int wp = block_id; wp < vertex_num_to_proc; wp += block_num)
    {
        ...
        { // global memory
            ...
            is_global = 1;
            // int nearest_prime = find_prime_by_warp(primes, prime_num, (neighbor_num * 3) / 2, warp_size); // neighbor_num must<37858284//?????????????????????

            // if (nearest_prime < 0)
            // {
            //     nearest_prime = find_prime_by_warp(primes, prime_num, neighbor_num, warp_size);
            // }
...
{ // warp_size=threadAlloc
    ...
    // for (unsigned int i = threadIdx.x; i < 32; i = i + blockDim.x)
    // {
    //     sum[i]=0;
    // }
    ...
    for (int wp = warp_id; wp < vertex_num_to_proc; wp += warp_num)
    {
        ...
        for (int thd = lane_id; thd < neighbor_num; thd += warp_size)
        {
            ...
            {
                cur_com_weight += wt;
                // atomicAdd(&sum[threadIdx.x/warp_size],wt);
            }
        }
        // __syncthreads(); //??????????????????
        // reduce
        ...
        // cur_com_weight=sum[threadIdx.x/warp_size];

        if (lane_id == warp_size - 1)
        {
            // atomicAdd(&In[vertex], cur_com_weight); // atomic??????
            In[vertex] = cur_com_weight;
            if (2 * (cur_com_weight - Self[vertex]) - (Ki - Self[vertex]) + (double)(min_Tot - (Tot[cur_com] - Ki)) * constant * Ki > 0)
            {
                active_set[vertex] = 0;
                // return ;
            }
        }
        // if (lane_id == 0 && cur_community[vertex] != cur_com)
        // {
        //     countMove[wp] += 1;
        // }
    }
...
{ // warp_size=threadAlloc
    ...
    for (int wp = block_id; wp < vertex_num_to_proc; wp += block_num)
    {
        ...
        {
            In[vertex] = sum;
            if (2 * (sum - Self[vertex]) - (Ki - Self[vertex]) + (double)(min_Tot - (Tot[cur_com] - Ki)) * constant * Ki > 0)
            {
                active_set[vertex] = 0;
            }
        }
                                                   ...
                                                   edge_t *edge_num_of_community, int community_num, int warp_size)
{
    ...
    {
        // edge_num_of_community[warp_id]=edge_numForWarp[0];
        // printf("%d %d\n",warp_id,num);
...
{ // a community allocate a block
    ...
    // extern __shared__ std::unordered_map<int, int> sharedMap;
...
{
    int t = (end.tv_sec * 1000 * 1000 + end.tv_usec - (start.tv_sec * 1000 * 1000 + start.tv_usec));
    if (isPrint)
    {
        printf("time:%dus\n", t);
    }
...
//         is_moved[vertex] = (is_neighbors_moved >= 1) * 1;//是否要计算In

//         int target_com_weight = (is_self_moved >= 1) * next_In[vertex] + (is_self_moved == 0) * In[vertex];
//         int tot_target_com = Tot[cur_community[vertex]];
//         active_set[vertex] = (is_neighbors_moved == 0)*(2 * (target_com_weight - Self[vertex]) - (Ki - Self[vertex]) + (double)(min_Tot - (tot_target_com - Ki)) * constant * Ki <= 0) * 1 + (is_neighbors_moved >= 1);
