...
struct degree_filter_in_range_gpu_id
{
    int lower;
    int upper;
    int gpu_id;
    int gpu_num;
    __host__ __device__ degree_filter_in_range_gpu_id(int l, int u,int gpu_id,int gpu_num)
        : lower(l), upper(u),gpu_id(gpu_id),gpu_num(gpu_num) {}
    __host__ __device__ bool operator()(const thrust::tuple<int, int>& t)
    {   
        int index = thrust::get<1>(t);
        int x=  thrust::get<0>(t);
        return (lower <= x && x <= upper)&&index%gpu_num==gpu_id;
    }
};

struct index_gpu_filter {
    int gpu_id;
    int gpu_num;
    __host__ __device__ index_gpu_filter(int gpu_id,int gpu_num)
        : gpu_id(gpu_id),gpu_num(gpu_num) {}
    __host__ __device__
    bool operator()(const int& index) const {
        return index % gpu_num == gpu_id;
    }
};
